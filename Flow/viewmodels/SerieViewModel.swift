...
class SerieViewModel: ObservableObject {
    ...
    @Published var series = [SerieParam]()
    
    func fetchUsers() async {
        
        // 1 - Url to fetch
        guard let url = URL(string: "https://api.airtable.com/v0/appMl82aryE3VlpSx/Random") else {
            return
        }
            ...
            if let error = error {
                ...
                do {
                    ...
                    for record in response.records {
                        DispatchQueue.main.async {
                            self.series.append(
                                SerieParam(title:record.fields.title, image: String(record.fields.image ?? "test"))
                            )
                        }
                    }
                } catch {

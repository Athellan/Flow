...
class SerieAddedViewModel: ObservableObject {
    
    
    @Published var seriesAdded = [SerieAdded]()
    
    func fetchUsers() async {
        
        // 1 - Url to fetch
        guard let url = URL(string: "https://api.airtable.com/v0/appMl82aryE3VlpSx/SerieAdded") else {
            return
        }
            ...
            if let error = error {
                ...
                do {
                    ...
                    for record in response.records {
                        DispatchQueue.main.async {
                            self.seriesAdded.append(
                                SerieAdded(title:record.fields.title, image: String(record.fields.image ?? "test"))
                            )
                        }
                    }
                } catch {

...
class FavoriteViewModel: ObservableObject {
    ...
    func fetchUsers() async {
        ...
        // 1 - Url to fetch
        guard let url = URL(string: "https://api.airtable.com/v0/appMl82aryE3VlpSx/Favorite") else {
            return
        }
                    ...
                    for record in response.records {
                        DispatchQueue.main.async {
                            self.favorites.append(
                                Favorite(title:record.fields.title, image: String(record.fields.image ?? "test"))
                            )
                        }
                    }
                } catch {

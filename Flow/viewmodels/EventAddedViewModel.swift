...
class EventAddedViewModel: ObservableObject {
    
    
    @Published var eventsAdded = [EventAdded]()
    
    func fetchUsers() async {
        
        // 1 - Url to fetch
        guard let url = URL(string: "https://api.airtable.com/v0/appMl82aryE3VlpSx/EventAdded") else {
            return
        }
            ...
            if let error = error {
                ...
                do {
                    ...
                    for record in response.records {
                        DispatchQueue.main.async {
                            self.eventsAdded.append(
                                EventAdded(title:record.fields.title, image: String(record.fields.image ?? "test"))
                            )
                        }
                    }
                } catch {
